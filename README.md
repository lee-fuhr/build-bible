...
**You stop reinventing.** 21 proven patterns with specific use cases and production citations. When a problem looks familiar, you reach for the pattern instead of rebuilding from memory.

**You catch mistakes before they cost you.** 11 named failure modes -- the 49-day research agent, the premature learning engine, the god file, the advisory illusion -- each with specific evidence of what it costs. Naming a thing is most of solving it.
...
- **15 principles** -- the non-negotiable rules: TDD, atomic operations, single source of truth, simplicity, checkpoint gates, enforce boundaries, and nine more. Each backed by evidence, not opinion.
- **21 reusable patterns** -- tiered agent model, approval queue, config-driven scaling, conductor pattern, event-sourced decision journal, and sixteen more. Each with a selection guide so you know when to reach for it.
- **11 anti-patterns with costs** -- specific names for the failure modes, specific evidence of what they cost. Recognize them fast.
